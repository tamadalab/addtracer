...
public class ArithmeticInstructionUpdateHandler extends TracerInstructionUpdateHandler{
    ...
    public InstructionList updateInstruction(InstructionHandle handle, UpdateData d){
        ...
        if(name.indexOf("add") > 0)       operand = "+";   // ¬≠§∑ªª
        else if(name.indexOf("sub")  > 0) operand = "-";   // ∞˙§≠ªª
        else if(name.indexOf("mul")  > 0) operand = "*";   // ≥›§±ªª
        else if(name.indexOf("div")  > 0) operand = "/";   // ≥‰§Íªª
        else if(name.indexOf("rem")  > 0) operand = "%";   // æÍÕæ
        ...
        else if(name.indexOf("neg")  > 0) operand = "~";   // …‰πÊ»ø≈æ
        else                              operand = name;
